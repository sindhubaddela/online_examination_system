...
BEGIN
    ...
    VALUES (NEW.email, 'INITIAL', 0, 0, 0, 0, CURRENT_TIMESTAMP);
END //
...
BEGIN
    -- Nested query to check if the email already exists in the user table
    IF NOT EXISTS (
        SELECT 1 
        FROM `user` 
        WHERE `email` = p_email
        ...
        INSERT INTO `user` (`name`, `email`, `password`, `gender`, `college`, `mob`) 
        VALUES (p_name, p_email, p_password, '', '', 0);
    ELSE
        -- Optionally handle the case when the email already exists (e.g., raise an error or return a message)
