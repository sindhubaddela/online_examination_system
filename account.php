...
// History Section
if (@$_GET['q'] == 2) {
    $email = $_SESSION['email']; // Ensure email is retrieved from session
    $q = mysqli_query($con, "SELECT * FROM history WHERE email='$email' ORDER BY date DESC") or die('Error197');
    echo '<div class="panel title">
    <table class="table table-striped title1">
    <tr style="color:black"><td><b>S.N.</b></td><td><b>Quiz</b></td><td><b>Question Solved</b></td><td><b>Right</b></td><td><b>Wrong<b></td><td><b>Score</b></td></tr>';
    $c = 0;
    ...
    while ($row = mysqli_fetch_array($q)) {
        ...
        $title = ""; // Initialize $title to avoid undefined variable warnings
        $q23 = mysqli_query($con, "SELECT title FROM quiz WHERE eid='$eid'") or die('Error208');
        if ($row2 = mysqli_fetch_array($q23)) {
            $title = $row2['title'];
        }
        
        $c++;
        echo '<tr><td>' . $c . '</td><td>' . (!empty($title) ? $title : 'Title Not Found') . '</td><td>' . $qa . '</td><td>' . $r . '</td><td>' . $w . '</td><td>' . $s . '</td></tr>';
    }
    echo '</table></div>';
}
...
if (@$_GET['q'] == 3) {
    ...
    while ($row = mysqli_fetch_array($q)) {
        ...
        $name = $gender = $college = ""; // Initialize variables
        ...
        <div class="panel-body">
            <form method="post" action="update.php" class="form-horizontal">
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-6">
                        <input id="mob" name="mob" type="text" class="form-control input-md" required>
                    </div>
                </div>
                <!-- Password Field -->
                <div class="form-group">
